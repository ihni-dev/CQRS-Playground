...
namespace CQRS.TaskManagementService.WebApi.Controllers
{
    public class BoardController : ApiControllerBase
    {
        ...
        public BoardController(ICommandBus commandBus, IQueryProcessor queryProcessor)
        {
            _commandBus = commandBus;
            _queryProcessor = queryProcessor;
        }

        [HttpGet, Route("{boardIdValue}")]
        public async Task<IActionResult> GetAsync(string boardIdValue)
        {
            var boardId = BoardId.With(boardIdValue);
            var board = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<BoardReadModel>(boardId),default(CancellationToken));
        ...
        public async Task<IActionResult> CreateAsync([FromBody]string boardName)
        {
            ...
            return CreatedAtAction(nameof(GetAsync), new { boardIdValue = boardId.ToString() }, new { id = boardId.Value, name = boardName });
        }
        
        [HttpPatch, Route("{boardIdValue}")]
